...
export const RHNotificationsPage = () => {
  ...
  const messages = rhNotifications.map((n, i) => ({
    id: n.id,
    from: n.type === 'error' ? 'Systeme' : 'Direction',
    email: 'systeme@rhpro.com',
    subject: n.title,
    preview: n.message,
    date: n.date,
    read: n.read,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white mb-2">Centre de Messages RH</h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base">Messagerie privee du service RH</p>
        </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {activeTab === 'templates' ? (
                <div className="p-2 space-y-1">
                  {['Demande de conge', 'Validation contrat'].map((t, i) => (
                    <div key={i} className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg cursor-pointer">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate">Modele RH...</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                  {currentMessages.length === 0 ? (
                    ...
                    currentMessages.map((msg: any) => (
                      <div key={msg.id} onClick={() => { setSelectedMessage(msg); setShowEmojiPicker(false) }} className={`p-3 sm:p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 ${selectedMessage?.id === msg.id ? 'bg-primary-50 dark:bg-primary-900/20 border-l-4 border-primary-500' : ''} ${!msg.read ? 'bg-slate-50 dark:bg-slate-800/50' : ''}`}>
                ...
                <div className="p-4 sm:p-6 border-t border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      ...
                      <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg ${showEmojiPicker ? 'bg-primary-100 dark:bg-primary-900/30' : ''}`}>
                        <Smile className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500" />
                      </button>
                    </div>
                    <button onClick={handleSendReply} disabled={!replyText.trim()} className="px-4 sm:px-6 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 flex items-center space-x-2 text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed">
