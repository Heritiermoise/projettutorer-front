  ...
  return (
    <div className="space-y-6">
              ...
              <div key={offre.id} className="bg-slate-50 dark:bg-slate-700/50 rounded-2xl p-6 border border-slate-200 dark:border-slate-600 hover:shadow-lg transition-all">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-600">
                  <div className="flex space-x-2">
                    ...
                    <button onClick={() => setOfferToDelete(offre)} disabled={deletingOfferId === offre.id} className="p-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-200 disabled:opacity-60" title="Supprimer définitivement">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex space-x-3 pt-4 sticky bottom-0 bg-white dark:bg-slate-800 pb-2">
                ...
                <button type="submit" className="flex-1 px-4 py-3 bg-primary-600 text-white rounded-xl hover:bg-primary-700">Creer l'offre</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <button onClick={() => setOfferToDelete(selectedOffre)} disabled={deletingOfferId === selectedOffre.id} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 disabled:opacity-60">
      ...
      <DeleteConfirmationModal
        ...
        description={offerToDelete
          ? `L'offre « ${offerToDelete.titre} » sera supprimée définitivement.${offerToDelete.nombre_candidatures > 0 ? ` Ses ${offerToDelete.nombre_candidatures} candidature(s) et les entretiens associés seront également supprimés.` : ''}`
          : ''}
