...
export const DirecteurNotificationsPage = () => {
  ...
  const messages = directeurNotifications.map((n, i) => ({
    id: n.id,
    from: n.type === 'error' ? 'Systeme' : 'Direction RH',
    ...
    read: n.read,
    starred: i === 0,
    type: n.type === 'error' ? 'system' : 'email',
  }))
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base">Communication avec l'equipe</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="lg:hidden p-4 border-b border-slate-200 dark:border-slate-700 flex items-center">
          {selectedMessage ? (
            <button onClick={() => { setSelectedMessage(null); setShowEmojiPicker(false) }} className="flex items-center space-x-2 text-amber-600">
            ...
            <div className="flex-1 overflow-y-auto">
              {activeTab === 'templates' ? (
                <div className="p-2 space-y-1">
                  {['Bienvenue', 'Rapport mensuel'].map((t, i) => (
                    <div key={i} className="p-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg cursor-pointer">
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate">Modele predefini...</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                  {currentMessages.length === 0 ? (
                    ...
                    currentMessages.map((msg: any) => (
                      <div key={msg.id} onClick={() => { setSelectedMessage(msg); setShowEmojiPicker(false) }} className={`p-3 sm:p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50 ${selectedMessage?.id === msg.id ? 'bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500' : ''} ${!msg.read ? 'bg-slate-50 dark:bg-slate-800/50' : ''}`}>
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      ...
                      <button 
                        ...
                        className={`p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg ${showEmojiPicker ? 'bg-amber-100 dark:bg-amber-900/30' : ''}`}
                      >
                    ...
                    <button 
                      ...
                      className="px-4 sm:px-6 py-2 bg-amber-600 text-white rounded-xl hover:bg-amber-700 flex items-center space-x-2 text-sm sm:text-base disabled:opacity-50 disabled:cursor-not-allowed"
                    >
